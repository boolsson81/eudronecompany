#!/usr/bin/env node
/**
 * Build MISSING_PRODUCT_TYPE.md via jsonld-product-scan report=missing_product_types.
 */
...
async function main() {
  const r = await fetch(`${URL}/functions/v1/jsonld-product-scan`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ shop_id: SHOP_ID, report: "missing_product_types" }),
  });
  ...
  const data = {
    source: json.source || "jsonld-product-scan → Supabase DB",
