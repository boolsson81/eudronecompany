#!/usr/bin/env node
/**
 * Deploy EuroDroneParts FAQ blog (theme + articles).
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/build-edp-faq-bundle.mjs
 *   node scripts/deploy-edp-faq-blog.mjs --dry-run
 *   node scripts/deploy-edp-faq-blog.mjs
 *   node scripts/deploy-edp-faq-blog.mjs --no-theme
 *
...
const REPORT = join(ROOT, "reports/edp-faq-deploy.json");
const BUNDLE = join(ROOT, "supabase/functions/_shared/edp-faq-bundle.json");
const THEME_DIR = join(ROOT, "shopify-theme/edp");
const SHOP = "ya1xhg-x6.myshopify.com";

const THEME_ASSETS = [
  "assets/edp-faq.css",
  "sections/edp-faq-blog.liquid",
  "sections/edp-faq-article.liquid",
  "templates/blog.vanliga-fragor.json",
  "templates/article.vanliga-fragor.json",
];
...
async function ensureBlog(blog) {
  ...
  const created = await gql(
    ...
    { blog: { title: blog.title, handle: blog.handle, templateSuffix: blog.templateSuffix || "vanliga-fragor" } },
  );
...
async function main() {
  ...
  console.log(`\nEDP FAQ blog deploy — ${dryRun ? "DRY RUN" : "LIVE"}\n`);

  let result = await invokeWorker({
    action: "edp_deploy_faq_blog",
    dry_run: dryRun,
  ...
  if (dryRun) {
    ...
    console.log("\nDeploy complete.");
    console.log(`Blog: https://eurodroneparts.se/blogs/${result.blog?.handle || "vanliga-fragor"}`);
  }
}
