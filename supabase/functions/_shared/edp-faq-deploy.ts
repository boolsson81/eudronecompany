...
import bundle from "./edp-faq-bundle.json" with { type: "json" };
...
async function deployThemeAssets(access: { domain: string; token: string; apiVersion: string }) {
  ...
  const assets = [
    { key: "assets/edp-faq.css", file: "assets/edp-faq.css" },
    { key: "sections/edp-faq-blog.liquid", file: "sections/edp-faq-blog.liquid" },
    { key: "sections/edp-faq-article.liquid", file: "sections/edp-faq-article.liquid" },
    { key: "templates/blog.vanliga-fragor.json", file: "templates/blog.vanliga-fragor.json" },
    { key: "templates/article.vanliga-fragor.json", file: "templates/article.vanliga-fragor.json" },
  ];
...
async function ensureBlog(
  access: { domain: string; token: string },
  blog: { handle: string; title: string; templateSuffix: string },
): Promise<string> {
  const found = await shopifyGraphql(access, `{ blogs(first: 50) { nodes { id handle title } } }`);
  ...
  const created = await shopifyGraphql(
    ...
    { blog: { title: blog.title, handle: blog.handle, templateSuffix: blog.templateSuffix } },
  );
...
export async function runEdpFaqBlogDeploy(
